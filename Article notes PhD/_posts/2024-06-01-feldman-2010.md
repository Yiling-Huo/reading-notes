...
Objective given a model (brain), m, is to minimise the average uncertainty (entropy) about some generalized sensory states. Generalised state = state + its velocity + acceleration + jerk + etc. Average uncertainty is proportional to the long-term average of surprise i.e. negative log-evidence. Minimising sensory entropy therefore corresponds to maximizing the accumulated log-evidence for a model of the world. 
